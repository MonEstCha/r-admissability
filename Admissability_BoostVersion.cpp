...
/************************************ Functions to approximate the r-admissability ************************************/
/**
 * Function performs DFS on the passed graph starting at vertex u
 * @param u, the vertex wrapped by a Boost graph vertex descriptor
 * @param g, the input graph
 * @param name_map, the map storing the name of the vertex
 * @param putToOrder, the array storing which vertices where already placed in the order
 * @param visited, the array storing which vertices were visited already in the course of the DFS or
 * 			are part of a path to a vertex not placed in the order yet
 * @param R, the maximum path length
 * @return 1, if the endpoint of a path is a vertex outside of the order, else 0
 *
 */
...
/**
 * Function iterates direct neighbors of vertex u and either increments the counter for the approximate of the r-admissability
 * if they are not placed in order yet or else if not only direct neighbors are considered calls execDFS routine on them
 * @param u, the vertex wrapped by a Boost graph vertex descriptor
 * @param g, the input graph
 * @param name_map, the map storing the name of the vertex
 * @param putToOrder, the array storing which vertices where already placed in the order
 ...
 * 			are part of a path to a vertex not placed in the order yet
 * @param R, the maximum path length
 * @return counter for approximate of the r-admissability
 *
 */
template < typename Graph, typename VertexNameMap >
